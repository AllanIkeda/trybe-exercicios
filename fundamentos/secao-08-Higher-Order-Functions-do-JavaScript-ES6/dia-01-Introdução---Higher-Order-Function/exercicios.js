...
const lotery = (mynumber, callback) => {
  ...
  return callback(mynumber, number)
    ? `sorteado  ${number} parabéns`
    : `sorteado  ${number} Perdeu`;
};
...
const dragonDMG = (personagem) => {
  ...
  console.log(`dano do dragão: ${returnDMG}`);
  return returnDMG;
};
...
const warriorDMG = (personagem) => {
  ...
  const returnDMG = maxDMG > minDMG ? maxDMG : minDMG;
  console.log(`dano do guerrerin: ${returnDMG}`);
  return returnDMG;
};
...
const mageDMG = (personagem) => {
  ...
  if (manaMage > 15) {
    ...
    console.log(`dano do mago(ado): ${returnDMG.damage}, mana: ${manaMage - returnDMG.manaGasta}/${manaMage}`);
    return returnDMG;
  }
...
const gameActions = {
  warrior: (warriorDMG) => {
    const dano = warriorDMG(warrior);
    dragon.healthPoints -= dano;
    warrior.damage = dano;
  },
  mage: (mageDMG) => {
  const dano = mageDMG(mage);
  const attDano = dano.damage;
  mage.damage = attDano;
  mage.mana -= dano.manaGasta;
  dragon.healthPoints -= attDano;
  },
  dragon: (dragonDMG) => {
    const dano = dragonDMG(dragon);
...
gameActions.warrior(warriorDMG);
gameActions.mage(mageDMG);
gameActions.dragon(dragonDMG);
console.log(gameActions.turnResults());
const oobj = gameActions.turnResults();
for ( obj in oobj) {
    for ( i in oobj[obj]) {
  console.log(`${obj}: ${oobj[obj]}`);
    }
};